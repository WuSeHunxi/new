...
- headers：对象，请求头信息（通常是身份验证的时候要在这里面配置信息，通常在 POST 请求中要添加 Content-Type，即：消息体的格式）
- body: 请求体的内容（要放在消息体中），必须匹配请求头中的 Content-Type
- mode：字符串，请求模式
...
- 当收到服务器的返回结果后，Promise 进入 resolved 状态(404 的时候也是 resolved 的状态)，状态数据为 Response 对象
- 当网络发生错误（或其他导致无法完成交互的错误）时，Promise 进入 rejected 状态(根本到达不了服务器：没有网络)，状态数据为错误信息

**Response 对象**

- ok：boolean，当响应消息码在 200~299 之间时为 true，其他为 false（只要有相应码了就是 resolved）
